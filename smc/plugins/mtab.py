...
from smc.models.table import CandidateEntity, Link, Table, TableRow
...
class MTabAssistant(Assistant):
    ...
    def create_sm_from_cta_cpa(
        ...
        for cid, qnode_id in types.items():
            ...
            curl = WDOnt.get_qnode_uri(qnode_id)

            try:
                cnode_label = f"{self.id2label[qnode_id]} ({qnode_id})"
            except KeyError:
                cnode_label = WDOnt.get_qnode_rel_uri(qnode_id)
            cnode = O.ClassNode(
                abs_uri=curl,
                rel_uri=WDOnt.get_qnode_rel_uri(qnode_id),
        ...
        for source_cid, target_cid, prop in rels:
            ...
            sm.add_edge(
                O.Edge(
                    source=source.id,
                    target=target.id,
                    abs_uri=WDOnt.get_prop_uri(prop),
                    rel_uri=WDOnt.get_prop_rel_uri(prop),
